...
const AdminHeader: React.FC = () => {
  ...
  const notificationContent = (
    <div style={{ width: 360, maxHeight: 400, overflow: 'auto', background: '#ffffff', border: '1px solid #e8eaed', borderRadius: 12, boxShadow: '0 8px 24px rgba(0,0,0,0.10)' }}>
      <div style={{
        ...
        {unreadCount > 0 && (
          <Button type="link" size="small" style={{ color: '#4f6ef7', fontSize: 12 }}>
      ...
      <List
        ...
        renderItem={(item) => (
          ...
          >
            <List.Item.Meta
              title={<Text strong={!item.isRead} style={{ fontSize: 14, color: '#1a1d26' }}>{item.title}</Text>}
              description={
                <Space direction="vertical" size={0}>
                  <Text style={{ fontSize: 13, color: '#6b7280' }}>{item.message}</Text>
                  <Text style={{ fontSize: 12, color: '#9ca3af' }}>{dayjs(item.createdAt).fromNow()}</Text>
                </Space>
              }
            />
          </List.Item>
  ...
  return (
    <div style={{
      ...
      {/* Left: logo placeholder — collapse moved to sidebar */}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <Popover content={notificationContent} trigger="click" placement="bottomRight" arrow={false}>
          <Badge count={unreadCount} size="small" offset={[-2, 2]} style={{ backgroundColor: '#ef4444' }}>
            <Button type="text" icon={<MessageOutlined style={{ fontSize: 18, color: '#6b7280' }} />} style={{ width: 40, height: 40 }} />
          </Badge>
        </Popover>
        ...
        <Dropdown menu={{ items: userMenuItems }} placement="bottomRight">
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.3 }}>
              ...
              <span style={{ fontSize: 11, color: '#9ca3af' }}>Quản trị viên</span>
            </div>
          </Space>
        </Dropdown>
      </div>
